...
from . import auto_rig_reset, auto_rig_datas, auto_rig, utils
from .utils import *
...
##########################  CLASSES  ##########################
class ARP_OT_GE_unset_softlink_bones(bpy.types.Operator):
...
class ARP_OT_GE_set_softlink_bones(bpy.types.Operator):
...
class ARP_OT_GE_unset_custom_bones(bpy.types.Operator):
...
class ARP_OT_GE_set_custom_bones(bpy.types.Operator):
...
class ARP_OT_GE_fix_rig(bpy.types.Operator):
...
class ARP_OT_GE_check_rig(bpy.types.Operator):
    #tooltip
...
class ARP_OT_toggle_action(bpy.types.Operator): 
...
class ARP_OT_set_humanoid_rig(bpy.types.Operator):
    #tooltip
...
class ARP_OT_export_fbx(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            collect_custom_bones(self.armature_name)
            max_display = 5
            ...
            if scn.arp_export_rig_type == "humanoid":
                ...
                # Bake Actions?
                if scn.arp_bake_actions and len(bpy.data.actions) > 0:
            ...
            print("\n..............................FBX Export..............................")
            #print(br)
            bpy.ops.arp_export_scene.fbx(filepath=self.filepath, use_selection=True, global_scale=scn.arp_global_scale, use_mesh_modifiers=True, use_armature_deform_only=True, add_leaf_bones=False, apply_unit_scale=True, bake_anim_simplify_factor=scn.arp_simplify_fac, mesh_smooth_type=scn.arp_mesh_smooth_type, use_tspace=scn.arp_use_tspace, primary_bone_axis=scn.arp_bone_axis_primary_export, secondary_bone_axis=scn.arp_bone_axis_secondary_export, shape_keys_baked_data=str(self.shape_keys_data), use_subsurf=False, use_custom_props=False)
...
def check_humanoid_limbs():
    ...
    if context.scene.arp_export_rig_type == "humanoid":
        ...
        if (len(limb_sides.arm_sides) > 2 or len(limb_sides.leg_sides) > 2 or len(limb_sides.head_sides) > 1 or len(limb_sides.ear_sides) > 2 or not found_root or rig.rig_spine_count < 3) or three_bones_leg:
            return False
...
def is_custom_bone(bone_name):
    if bone_name in custom_bones_list:
        return True
    else:
        return False
...
def _add_ik_bones():
    ...
    for side in sides:
        ...
        if foot:
            ...
            if get_edit_bone("ik_foot_root") == None:
                ...
                new_bone.head, new_bone.tail = Vector((0,0,0)), Vector((0,1,0))
                new_bone.roll = 0.0
    ...
    hand_r = get_edit_bone("hand" + sides[1])
    if hand_r:
        # create the IK root bone
    ...
    if scn.arp_ue_ik_anim:
        # feet
        for side in sides:
            foot_ik = get_pose_bone("ik_foot"+side)
            if foot_ik == None:
                continue

            """
            # armature constraint
            cns = foot_ik.constraints.new("ARMATURE")
            t = cns.targets.new()
            t.target = skel
            t.subtarget = "foot"+side
            """
            # copy transform constraint
            cns = foot_ik.constraints.new("COPY_TRANSFORMS")
            cns.target = skel
            cns.subtarget = "foot"+side


        # hands
        for side in sides:
            hand_ik = get_pose_bone("ik_hand"+side)
            if hand_ik == None:
                continue

            """
            # armature constraint
            cns = hand_ik.constraints.new("ARMATURE")
            t = cns.targets.new()
            t.target = skel
            t.subtarget = "hand"+side
            """
            # copy transform constraint
...
def _set_mannequin_orientations(self):
    ...
    for e_bone in rig_humanoid.data.edit_bones:
        if not is_custom_bone(e_bone.name) and not is_bend_bone(e_bone.name):
    ...
    for bone_name in bones_transforms_dict:
        ...
        if bone_to_create_name != "" and get_edit_bone(bone_to_create_name + "_orient") == None:
            ...
            new_bone = bpy.context.active_object.data.edit_bones.new(bone_to_create_name + "_childof")
    ...
    #rename all
    for ebone in rig_humanoid.data.edit_bones:
        # exclude custom bones, bend bones, facial bones, tail bones, breast bones, spline bones, ear bones
        if not is_custom_bone(ebone.name) and not is_bend_bone(ebone.name) and not is_facial_bone(ebone.name) and not ebone.name.startswith('c_tail_') and not ebone.name.startswith('c_breast_') and not "arp_spline" in ebone.keys() and not ebone.name.startswith("c_ear_"):
...
def _set_humanoid_rig(armature_name, armature_add_name, manual_set_debug, self):
    ...
    # Initialize bones dict: name, head (blank coordinates), tail (blank coordinates), roll (blank coordinates), use_deform (always True)
    ...
    if arp_armature.arp_secondary_type == "TWIST_BASED":
        for side in [".l", ".r"]:
            for bname in ['arm', 'forearm', 'thigh', 'leg']:
                ...
                sftlink = c_stretch_name in softlink_bones
                ...
                #bones_dict[b_parent_n]["softlink"] = sftlink
            ...
            for side in [".l", ".r"]:
                ...
                if bone_name == "thigh_twist" or bone_name == "leg_twist":
                    ...
                    if bone_name == "leg_twist":
                        ...
                        twist_bones_dict[twist_bone_name] = {"head":twist_bone.head.copy(), "tail":twist_bone.tail.copy(), "roll":twist_bone.roll, "parent":b_parent, "softlink":twist_bone_name in softlink_bones}
                        ...
                        if twist_bones_amount > 1:
                            for t_idx in range(2, twist_bones_amount+1):
                                ...
                                b_parent = 'leg_stretch'+side
                                twist_bones_dict[leg_twist_idx_name] = {"head":leg_twist_idx.head.copy(), "tail":leg_twist_idx.tail.copy(), "roll":leg_twist_idx.roll, "parent":b_parent, "softlink":leg_twist_idx_name in softlink_bones}
                    ...
                    if bone_name == "thigh_twist":
                        ...
                        if twist_bones_amount > 1:
                            for t_idx in range(2, twist_bones_amount+1):
                                ...
                                b_parent = 'thigh_stretch'+side
                                twist_bones_dict[thigh_twist_idx_name] = {"head":thigh_twist_idx.head.copy(), "tail":thigh_twist_idx.tail.copy(), "roll":thigh_twist_idx.roll, "parent":b_parent, "softlink":thigh_twist_idx_name in softlink_bones}
                    ...
                    if bone_name == "forearm_twist":
                        ...
                        if twist_bones_amount > 1:
                            for t_idx in range(2, twist_bones_amount+1):
                                ...
                                b_parent = 'forearm_stretch'+side
                                twist_bones_dict[forearm_twist_idx_name] = {"head":forearm_twist_idx.head.copy(), "tail":forearm_twist_idx.tail.copy(), "roll":forearm_twist_idx.roll, "parent":b_parent, "softlink":forearm_twist_idx_name in softlink_bones}
                    ...
                    if bone_name == "c_arm_twist_offset":
                        ...
                        if twist_bones_amount > 1:
                            for t_idx in range(2, twist_bones_amount+1):
                                ...
                                b_parent = 'arm_stretch'+side
                                twist_bones_dict[arm_twist_idx.name] = {"head":arm_twist_idx.head.copy(), "tail":arm_twist_idx.tail.copy(), "roll":arm_twist_idx.roll, "parent":b_parent, "softlink":arm_twist_idx_name in softlink_bones}


    # Store custom bones and extra limbs
    for edit_bone in arp_armature.data.edit_bones:
        bone_softlink = edit_bone.name in softlink_bones
    ...
    for bname in bones_dict:
        ...
        bone_softlink = _bname in softlink_bones
        side = bname[-2:]
    ...
    if is_facial_enabled(arp_armature):
        ...
        jaw_softlink = 'jawbone.x' in softlink_bones
        ...
        for side in sides:
            eyelid_softlink = 'c_eyelid_top_01'+side in softlink_bones
    ...
    if neck_ref:
        ...
        if neck_count > 1:
            for idx in range(1, neck_count):
                ...
                subneck_softlink = subneck_deform_name in softlink_bones
    ...
    for side in limb_sides.wing_sides:
        ...
        for bname in arm_feathers+forearm_feathers+hand_feathers:
            ...
            feather_softlink = ebone.name in softlink_bones
    ...
    for bname in bones_dict:
        ...
        new_bone = bpy.context.active_object.data.edit_bones.new(bname)
    ...
    if len(c_stretch_dict) > 0:
        for stretch_bone_name in c_stretch_dict:
            new_bone = bpy.context.active_object.data.edit_bones.new(stretch_bone_name)
    ...
    def find_matching_parent(par = None, item = None):
        ...
        if 'neck' in par:
            _parent = 'neck.x'
        if 'head' in par or ('skull' in par and scn.arp_full_facial == False and is_facial_enabled(arp_armature) == False):
            _parent = 'head.x'
        if par.startswith("c_skull_") and scn.arp_full_facial == False:
            _parent = "head.x"
        if 'shoulder' in par:
    ...
    for b in humanoid_armature.data.edit_bones:
        ...
        if b.name[:-2] == 'arm_stretch':
            if scn.arp_export_twist and b["softlink"] == 1:# if softlink, need to use actual arm_stretch position
            ...
            if scn.arp_export_twist and b["softlink"] == 1:# if softlink, need to use actual thigh_stretch position
                b.head = bones_dict[b.name]["head"]
            else:
    ...
    for pb in humanoid_armature.pose.bones:
        ...
        # thigh_stretch and arm_stretch needs to be at actual position if softlinked
        if pb.name.startswith("thigh_stretch.") or pb.name.startswith("arm_stretch."):
    ...
    for obj_name in collected_meshes:
        ...
        for modif in obj.modifiers:
            ...
            if jawbone_vgroup:
                ...
                #print('Renamed vgroup "jawbone.x" to "c_jawbone.x"')
                ...
                for grp in vert.groups:
                    ...
                    if not scn.arp_full_facial:
                        ...
                        if is_facial_enabled(arp_armature):
                            ...
                            # facial jaw
                            transfer_weight(object=obj, vertice=vert, vertex_weight=weight, group_name=grp_name, list=facial_transfer_jaw, target_group = jawbone_name)
    ...
    for mesh in collected_meshes:
        ...
        if len(obj.keys()) > 0:
            if "arp_parent_bone" in obj.keys():
                ...
                if get_object("rig_humanoid").data.bones.get(b_parent):
                    parent_found = True
                ...
                obj.parent = get_object("rig_humanoid")
...
def spline_ik_export_to_dict(name, side, dict):
    ...
    for i in range(1, amount+1):
        ...
        bone_softlink = bname in softlink_bones
...
def _set_mped_rig(armature_name, armature_add_name, manual_set_debug, self):
    ...
    if is_deforming("root.x"):
        ...
        if c_root_master.parent:
            if is_custom_bone(c_root_master.parent.name):
                parent_name = c_root_master.parent.name
        bones_coords["root.x"] = {"head":ebone.tail.copy(), "tail":ebone.head.copy(), "roll":ebone.roll, "parent":parent_name, "x_axis":ebone.x_axis.copy(), "arp_bone":ebone.name, "softlink":"root.x" in softlink_bones}
    ...
    for idx in range(1, spine_count+1):
        ...
        if is_deforming(spine_name):
            ...
            if idx == 1:
                b_parent = "root.x"
            bones_coords[spine_name] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":b_parent, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}
    ...
    for dupli in limb_sides.head_sides:
        ...
        if neck and is_deforming(neck_name):
            ...
            bones_coords[neck.name] = {"head":neck.head.copy(), "tail":neck.tail.copy(), "roll":neck.roll, "parent":b_parent_name, "arp_bone":neck.name, "softlink":neck.name in softlink_bones}
            ...
            if neck_count > 1:
                for idx in range(1, neck_count):
                    ...
                    subneck_cont = get_edit_bone(subneck_cont_name)
                    subneck_parent_name = subneck_cont.parent.name
                    bones_coords[subneck_cont_name] = {"head":subneck_deform.head.copy(), "tail":subneck_deform.tail.copy(), "roll":subneck_deform.roll, "parent":subneck_parent_name, "arp_bone":subneck_deform.name, "softlink":subneck_deform.name in softlink_bones}
            ...
            if get_edit_bone('c_neck_01' + dupli) != None and is_deforming('c_neck_01' + dupli):
                ...
                bones_coords[neck01.name] = {"head":neck01.head.copy(), "tail":neck01.tail.copy(), "roll":neck01.roll, "parent":b01_parent, "arp_bone":neck01.name, "softlink":neck01.name in softlink_bones}
        ...
        if get_edit_bone('head' + dupli) != None and is_deforming('head' + dupli):
            ...
            bones_coords[headb.name] = {"head":headb.head.copy(), "tail":headb.tail.copy(), "roll":headb.roll, "parent":b_parent, "arp_bone":headb.name, "softlink":headb.name in softlink_bones}
        ...
        for i in range(1,4):
            if get_edit_bone("c_skull_0" + str(i) + dupli) != None:
                ...
                if is_deforming(skull.name):
                    bones_coords[skull.name] = {"head":skull.head.copy(), "tail":skull.tail.copy(), "roll":skull.roll, "parent":skull.parent.name, "arp_bone":skull.name, "softlink":skull.name in softlink_bones}
    ...
    for dupli in facial_duplis:
        ...
        if get_edit_bone(bname) != None:
            ebone = get_edit_bone(bname)
            bones_coords[ebone.name] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":"None", "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}

        for side in sides:
            bname = "c_eye_target" + dupli[:-2] + side
            if get_edit_bone(bname) != None:
                ebone = get_edit_bone(bname)
                bones_coords[ebone.name] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":ebone.parent.name, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}
        ...
        for side in sides:
            ...
            if get_edit_bone(bname):
                ...
                if get_edit_bone("c_skull_02" + dupli):
                    ...
                    b_parent = "head" + dupli
                bones_coords[ebone.name] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":b_parent, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}
        ...
        for bname in bones_parent_dict:
            ...
            ebone = get_edit_bone(bname)
            bones_coords[bname] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":bones_parent_dict[bname], "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}
    ...
    # Tail
    if get_data_bone("c_tail_00.x"):
        for i in range(0, arp_armature.rig_tail_count):
        ...
        for side in limb_sides.leg_sides:
            if is_deforming(bone_name + side):
                ...
                if bone_name.startswith('c_toes'):
                    ...
                    bones_coords["toes_01"+side] = {"head":toes_01.head.copy(), "tail":toes_01.tail.copy(), "roll":toes_01.roll, "parent":"foot"+side, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}
                ...
                if "thigh_stretch" in bone_name:
                    ...
                    thigh_softlink = ebone.name in softlink_bones
                    if thigh_softlink:
                    ...
                    if arp_armature.arp_secondary_type == "TWIST_BASED":
                        bones_coords["c_thigh_stretch"+side] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":"thigh_stretch"+side, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}

                if "leg_stretch" in bone_name:
                    b_parent = "thigh_stretch" + side
                    #print("  set parent", b_parent, bone_name)
                    bones_coords["leg_stretch"+side] = {"head":ebone.head.copy(), "tail":get_edit_bone("leg_twist" + side).tail.copy(), "roll":ebone.roll, "parent":b_parent, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}

                    if arp_armature.arp_secondary_type == "TWIST_BASED":
                        bones_coords["c_leg_stretch"+side] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":"leg_stretch"+side, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}
                ...
                if scn.arp_export_twist or arp_armature.arp_secondary_type == "TWIST_BASED":
                    ...
                    if "leg_twist" in bone_name:
                        # first leg twist bone (default)
                        b_parent = "leg_stretch"+side
                        bones_coords["leg_twist"+side] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":b_parent, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}
                        ...
                        if twist_bones_amount > 1:
                            for t_idx in range(2, twist_bones_amount+1):
                                ...
                                b_parent = 'leg_stretch'+side
                                bones_coords[leg_twist_idx.name] = {"head":leg_twist_idx.head.copy(), "tail":leg_twist_idx.tail.copy(), "roll":leg_twist_idx.roll, "parent":b_parent, "arp_bone":leg_twist_idx.name, "softlink":leg_twist_idx.name in softlink_bones}

                    if "thigh_twist" in bone_name:
                        # first thigh twist bone (default)
                        b_parent = "thigh_stretch"+side
                        bones_coords["thigh_twist"+side] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":b_parent, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}
                        # multiple thigh twist bones
                        if twist_bones_amount > 1:
                            for t_idx in range(2, twist_bones_amount+1):
                                thigh_twist_idx = get_edit_bone('thigh_twist_'+str(t_idx)+side)
                                b_parent = 'thigh_stretch'+side
                                bones_coords[thigh_twist_idx.name] = {"head":thigh_twist_idx.head.copy(), "tail":thigh_twist_idx.tail.copy(), "roll":thigh_twist_idx.roll, "parent":b_parent, "arp_bone":thigh_twist_idx.name, "softlink":thigh_twist_idx.name in softlink_bones}
        ...
        for side in limb_sides.arm_sides:
            if is_deforming(bone_name + side):
                ...
                if "arm_stretch" in bone_name and not "forearm_stretch" in bone_name:
                    arm_softlink = ebone.name in softlink_bones
                    ...
                    if arp_armature.arp_secondary_type == "TWIST_BASED":
                        bones_coords["c_arm_stretch"+side] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":"arm_stretch"+side, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}

                if "forearm_stretch" in bone_name:
                    bones_coords["forearm_stretch"+side] = {"head":ebone.head.copy(), "tail":get_edit_bone("forearm_twist" + side).tail.copy(), "roll":ebone.roll, "parent":"arm_stretch"+side, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}

                    if arp_armature.arp_secondary_type == "TWIST_BASED":
                        bones_coords["c_forearm_stretch"+side] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":"forearm_stretch"+side, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}

                if bone_name == "hand":
                    bones_coords[ebone.name] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":"forearm_stretch"+side, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}
                ...
                if scn.arp_export_twist or arp_armature.arp_secondary_type == "TWIST_BASED":
                    ...
                    if "c_arm_twist" in bone_name:
                        # first arm twist bone (default)
                        b_parent = "arm_stretch" + side
                        bones_coords["arm_twist" + side] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":b_parent, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}
                        # multiple arm twist bones
                        if twist_bones_amount > 1:
                            for t_idx in range(2, twist_bones_amount+1):
                                arm_twist_idx = get_edit_bone('arm_twist_'+str(t_idx)+side)
                                b_parent = 'arm_stretch'+side
                                bones_coords[arm_twist_idx.name] = {"head":arm_twist_idx.head.copy(), "tail":arm_twist_idx.tail.copy(), "roll":arm_twist_idx.roll, "parent":b_parent, "arp_bone":arm_twist_idx.name, "softlink":arm_twist_idx.name in softlink_bones}

                    if "forearm_twist" in bone_name:
                        # first forearm twist bone (default)
                        b_parent = "forearm_stretch" + side
                        bones_coords["forearm_twist" + side] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":b_parent, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}
                        # multiple forearm twist bones
                        if twist_bones_amount > 1:
                            for t_idx in range(2, twist_bones_amount+1):
                                forearm_twist_idx = get_edit_bone('forearm_twist_'+str(t_idx)+side)
                                b_parent = 'forearm_stretch'+side
                                bones_coords[forearm_twist_idx.name] = {"head":forearm_twist_idx.head.copy(), "tail":forearm_twist_idx.tail.copy(), "roll":forearm_twist_idx.roll, "parent":b_parent, "arp_bone":forearm_twist_idx.name, "softlink":forearm_twist_idx.name in softlink_bones}
    ...
    for side in limb_sides.arm_sides:
        for b_finger in fingers_deform:
            ...
            if is_deforming(finger_name):
                ...
                bones_coords[finger_name] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":b_parent, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}
    ...
    for side in limb_sides.wing_sides:
        ...
        for bname in arm_feathers+forearm_feathers+hand_feathers:
            ...
            bones_coords[bname] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":b_parent, "arp_bone":ebone.name, "softlink":ebone.name in softlink_bones}
    ...
    if scn.arp_keep_bend_bones:
        for eb in arp_armature.data.edit_bones:
            ...
            # Store bone in dict
            bones_coords[eb.name] = {"head":eb.head.copy(), "tail":eb.tail.copy(), "roll":eb.roll, "parent":b_parent, "arp_bone":eb.name, "softlink":eb.name in softlink_bones}
    ...
    for cc_bone in arp_armature.data.edit_bones:
        if is_custom_bone(cc_bone.name):
            ...
            #Store bone in dict
            bones_coords[cc_bone.name] = {"head":cc_bone.head.copy(), "tail":cc_bone.tail.copy(), "roll":cc_bone.roll, "parent":b_parent, "custom_props":cprops, "arp_bone":cc_bone.name, "softlink":cc_bone.name in softlink_bones}
    ...
    def substract_weight(object=None, vertice=None, vertex_weight=None, group_name=None, dict=None):
        ...
        if grp_name_base in dict:
            if dict[grp_name_base].endswith('.x'):
                side = ''
    ...
    def clamp_weights(object=None, vertice=None, vertex_weight=None, group_name=None, dict=None):
        ...
        if grp_name_base in dict:
            ...
            if object.vertex_groups.get(_target_group) != None:
                for grp in vertice.groups:
                    #print(grp.group)
                    if grp.group > len(object.vertex_groups)-1:# Bug, some vertices may have groups ID that don't exist
                        continue
                    cur_grp_name = object.vertex_groups[grp.group].name
                    if cur_grp_name == _target_group:
                        target_weight = grp.weight
...
def rename_for_ue():
    ...
    if scene.arp_rename_for_ue:
        ...
        for pbone in rig_humanoid.pose.bones:
            ...
            if is_bend_bone(pbone.name) or is_custom_bone(pbone.name):
                continue
...
class ARP_OT_GE_export_fbx_panel(bpy.types.Operator, ExportHelper):
    ...
    def draw(self, context):
        ...
        def anim_export_ui():
            ...
            if scn.arp_bake_actions:
                row.enabled = True
            if scn.arp_bake_actions == False:# or scn.arp_bake_only_active:
                row.enabled = False
            row.prop(scn, "arp_only_containing", text="Only Containing:")

            row1 = row.row()
            if scn.arp_bake_actions and scn.arp_only_containing:
                row1.enabled = True
            else:
                row1.enabled = False
            row1.prop(scn, "arp_export_name_string", text="")
            row.prop(scn, "arp_see_actions", text="", icon="HIDE_OFF")
            if scn.arp_see_actions and scn.arp_bake_actions:
                if len(bpy.data.actions) > 0:
                    found_one_act = False
                    for act in bpy.data.actions:
...
def register():
    ...
    bpy.types.Scene.arp_export_twist = bpy.props.BoolProperty(name='Export Twist', description="If enabled, twist bones are exported. If disabled, they're not exported and the twist weights are transferred to the main weights.\nIf multiple twist bones are used, this is enabled automatically", default=True)
    bpy.types.Scene.arp_twist_fac = bpy.props.FloatProperty(name='Twist Amount', description="Influence of the twist bone, only if 1 twist bone if used. Generally 0.5 gives better results since game engines do not support dual quaternions\nIn case of multiple twist bones, this value is ignored since it's no more necessary", default = 0.5, min=0.0, max=1.0)
    bpy.types.Scene.arp_keep_bend_bones = bpy.props.BoolProperty(name='Advanced', description='Export secondary controllers ("_bend" bone). Useful for cartoons rigs. \nWarning, in Additive mode, it may change a little the bend bones skin weights. In Twist mode, arms and legs secondary controllers are always exported', default=False)
    ...
    bpy.types.Scene.arp_ue_ik_anim = bpy.props.BoolProperty(name='Animated IK Bones', description="Animated IK bones: the IK bones will inherit the feet and hands motions", default=False)
    bpy.types.Scene.arp_ue_root_motion = bpy.props.BoolProperty(name='Root Motion', description='The "c_traj" controller animation will be baked to the root node for root motion in Unreal', default=False)#, update=update_root_motion_btn)
    bpy.types.Scene.arp_only_containing = bpy.props.BoolProperty(name='Export Only Containing:', description='Export only actions with names containing the given text\ne.g. to export only actions associated to "Bob", name the actions "Bob_ActionName"' , default=False)
    ...
    bpy.types.Scene.arp_frame_range_type = bpy.props.EnumProperty(items=(('FULL', 'All', 'Export all frames'), ('CUSTOM', 'Custom', 'Export using given start/end frames'), ('SCENE', 'Scene', 'Export using start/end frames as set in the output tab'), ('MARKERS', 'Markers', 'Export frames within "Start" and "End" markers\nIf the action has no markers, all frames are exported')), name='Frame Range Type', default='FULL')
    bpy.types.Scene.arp_export_start_frame = bpy.props.IntProperty(name='Start', description='Start frame' , default=0)
    bpy.types.Scene.arp_export_end_frame = bpy.props.IntProperty(name='End', description='End frame' , default=250)
    bpy.types.Scene.arp_bake_only_active = bpy.props.BoolProperty(name='Only Active', description='Only export the active action' , default=True, update=update_only_active)
    ...
    bpy.types.Scene.arp_use_tspace = bpy.props.BoolProperty(name="Tangent Space", default=False, description="Add binormal and tangent vectors, together with normal they form the tangent space (will only work correctly with tris/quads only meshes!)")
    ...
    bpy.types.Scene.arp_fix_fbx_rot = bpy.props.BoolProperty(name='Fix Rotations', description='Add invisible jitter to the bones position and rotation to prevent rotations issues, by avoiding 180 angles between the child and parent' , default=False)
    bpy.types.Scene.arp_fix_fbx_matrix = bpy.props.BoolProperty(name='Fix Matrices', description='Use an alternative method to evaluate the bones matrices, to prevent shaky bones' , default=True)
    bpy.types.Scene.arp_init_fbx_rot = bpy.props.BoolProperty(name='Initialize Fbx Armature Rotation', description='Export armature with rotations values set to 0', default = False)#, update=update_init_rot_btn)
    bpy.types.Scene.arp_init_fbx_rot_mesh = bpy.props.BoolProperty(name='Initialize Fbx Meshes Rotation', description='Export meshes with rotations values set to 0', default=False)#, update=update_init_rot_btn)
    bpy.types.Scene.arp_ge_sel_only = bpy.props.BoolProperty(name='Selected Objects Only', description='Export only selected objects, instead of exporting all skinned meshes' , default=False)
    bpy.types.Scene.arp_ge_sel_bones_only = bpy.props.BoolProperty(name='Selected Bones Only', description='Export only selected bones (only deforming bones must be selected)' , default=False)
    ...
    bpy.types.Scene.arp_export_noparent = bpy.props.BoolProperty(name="No Parent", description="Allow correct export of bones stretch/scale by exporting a flat hierarchy (no parenting).\nWarning, it breaks retargetting features in game engines since the bones hierarchy is not preserved", default=False)
    bpy.types.Scene.arp_export_show_panels = bpy.props.EnumProperty(name="Panel Type", items=(('RIG', "Rig", ""), ('ANIM', "Animations", ""),('MISC', "Misc", "")),default='RIG')
    bpy.types.Scene.arp_export_rig_name = bpy.props.StringProperty(name="Rig Name", default="root", description="Name of the exported skeleton")
